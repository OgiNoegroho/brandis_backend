...
export class DistributionModel {
  ...
  async createDistributionAndFakturTransaction(
    ...
    try {
      ...
      const distributionResult = await client.query(distributionQuery, [distribution.outlet_id]);
      const distributionId = distributionResult.rows[0].id;

      // 2. Insert distribution details and process batches
      for (const detail of details) {
        const detailQuery = `INSERT INTO brandis.detail_distribusi (distribusi_id, batch_id, kuantitas_terjual) VALUES ($1, $2, $3)`;
        await client.query(detailQuery, [distributionId, detail.batch_id, detail.kuantitas_terjual]);

        const batchUpdateQuery = `UPDATE brandis.batch SET kuantitas = kuantitas - $1 WHERE id = $2 AND kuantitas >= $1`;
        const batchUpdateResult = await client.query(batchUpdateQuery, [detail.kuantitas_terjual, detail.batch_id]);
        ...
        await client.query(stockUpsertQuery, [distribution.outlet_id, detail.batch_id, detail.kuantitas_terjual]);
      }
      ...
      `;
      const amountResult = await client.query(totalAmountQuery, [distributionId]);
      ...
      const fakturId = fakturResult.rows[0].id;
